...
def quiz_summary(request):
    ...
    # Use Django's aggregation to count correct and incorrect answers
    correct_answers = sessions.filter(is_correct=True).aggregate(correct_count=Count('id'))['correct_count']
    incorrect_answers = sessions.filter(is_correct=False).aggregate(incorrect_count=Count('id'))['incorrect_count']

    details = []

    # Iterate over each session entry to show the question, user's answer, and the correct answer
    for session in sessions:
        ...
        # Add correct answer for highlighting
        question_details["correct_option"] = session.question.correct_option
